        ...
        success: function (data) {
            ...
            var listData=  data.response;
            	...
            	if(listData.length==0){
            		tableBody.append('<tr><td colspan="12" style="text-align: center">' +data.message+' </td></tr>' );
            	}	
            	else{
            		$(listData).each(function (index, element) {
                        tableBody.append('<tr><td>'+element.firstName+'</td><td>'+element.lastName+'</td><td>'+element.userName+'</td><td>'+element.roleName+'</td><td><button onclick = updateStudent('+element.id+',"'+element.firstName+'","'+element.lastName+'","'+element.role+'","'+element.userName+'","'+element.password+'","'+element.roleName+'")>Update</button> | <button onclick = "deleteStudent('+element.id+')">Delete</button></td></tr>');
                    })  	
            	}
...
function updateStudent(id,firstName,lastName,role,userName,password,roleName){ 
